...
The panda glyph is a **vector redraw of a reference supplied by the site
owner, who generated it themselves**. It is not third-party stock artwork and
carries no external licence. It was redrawn rather than embedded as a raster
image so it scales from a 16px favicon to the 1200px share card, and inherits
`currentColor` instead of being locked to one palette.

The geometry is duplicated in three places and must be kept in sync:
`src/components/brand/PandaMark.tsx`, `public/favicon.svg` and
`public/og-image.svg`. After editing any of them, run `npm run og` to
...
| File | What it is |
| --- | --- |
| `public/favicon.svg` | Panda glyph on a light plate |
...
For brand artwork, replace `public/favicon.svg`, `public/og-image.svg` and
`src/components/brand/PandaMark.tsx` together, then run `npm run og` to
regenerate the PNG derivatives.
